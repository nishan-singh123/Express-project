...
const getUsers = async (query) => {
    ...
    if (name) filters.name = { $regex: name, $options: "i" }; // Ilike match (case sensitive match)
...
const updateUser = async (id, data, authUser) => {
    if (authUser._id !== id && !authUser.roles.includes(ROLE_ADMIN)) throw {
        status: 403,
...
export default { createUser, getUsers, updateProfileImage, getUserById, updateUser, deleteUser, updateUserRoles };